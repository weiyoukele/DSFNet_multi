...
def inference_worker(args):
    ...
    # --- 预处理所需的常量 ---
    resized_h, resized_w = dataset_info['default_resolution'] # 注意：COCO是W,H，这里需要确认
    resized_h, resized_w = resized_w, resized_h # DSFNet的default_resolution是[W, H]
    ...
    for frame_path in frame_paths:
        ...
        save_predictions_as_yolo(
            predictions=dets,
            resized_shape=(resized_h, resized_w), # 传递正确的resize尺寸
...
def parallel_inference_and_evaluate(opt, modelPath):
    ...
    print(f"总共找到 {len(all_frame_paths)} 帧图像，将分配给 {num_gpus_needed} 张 GPU。")
