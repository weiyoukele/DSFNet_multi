...
class DetectionEvaluator:
    ...
    # ==================== 新增以下方法 ====================
    def save_summary_to_file(self, overall_metrics, file_path):
        """将 print_overall_results 的内容保存到文本文件"""
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write("="*88 + "\n")
            f.write("总体评估结果 (各视频性能指标)\n")
            f.write("="*88 + "\n")
            ...
            f.write(f"{'视频名称':<20} {'召回率':<8} {'精确率':<8} {'虚警率':<8} {'时序一致性':<10} {'TP':<6} {'FP':<6} {'FN':<6} {'帧数':<6}\n")
            f.write("-" * 88 + "\n")
            ...
            for video_name in sorted(self.video_results.keys()):
                ...
                if 'overall' in metrics:
                    ...
                    temporal_consistency = r['temporal_consistency']
                    stability_mark = "✓" if temporal_consistency >= self.stability_threshold else " "
                    f.write(f"{video_name:<20} {r['recall']:<8.3f} {r['precision']:<8.3f} {r['false_alarm_rate']:<8.3f} "
                            f"{temporal_consistency:<9.3f}{stability_mark} {r['tp']:<6} {r['fp']:<6} {r['fn']:<6} {frames:<6}\n")
            ...
            if 'overall' in overall_metrics:
                ...
                r = overall_metrics['overall']
                video_count = r.get('video_count', 0)
                total_frames = sum(vr['frames_processed'] for vr in self.video_results.values())
                
                f.write(f"{'总体平均':<20} {r['recall']:<8.3f} {r['precision']:<8.3f} {r['false_alarm_rate']:<8.3f} "
                        f"{r['temporal_consistency']:<10.3f} {r['tp']:<6} {r['fp']:<6} {r['fn']:<6} {total_frames:<6}\n")
                
                f.write(f"\n详细统计:\n")
                f.write(f"总真实目标数: {r['total_gt']}\n")
                f.write(f"总预测目标数: {r['total_pred']}\n")
                f.write(f"总处理帧数: {total_frames}\n")
                f.write(f"IoU阈值: {self.iou_threshold}\n")
                f.write(f"评估视频数: {video_count}\n")
                f.write(f"时序一致性IoU阈值: {r['consistency_iou_threshold']}\n")
                f.write(f"时空序列稳定性: {r['spatiotemporal_stability']:.3f} ({r['stable_videos']}/{video_count} 个视频超过{r['stability_threshold']:.0%}阈值)\n")
                ...
                f.write(f"\n时序一致性统计:\n")
                f.write(f"  目标数≤5帧: {total_consistent_le5}/{total_frames_le5} ({overall_le5_rate:.3f}) - 要求100%检出\n")
                f.write(f"  目标数>5帧: {total_consistent_gt5}/{total_frames_gt5} ({overall_gt5_rate:.3f}) - 要求80%检出\n")
    # ========================================================
def main():
    # ==================== 配置区域 ====================
    config = {
